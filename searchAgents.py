...
def foodHeuristic(state, problem):
    ...
    for i in range(0, len(foods) - 1):
        ...
        for j in range(i+1, len(foods)):
            # Distance entre 2 elements
            x1, y1 = foods[i]
            x2, y2 = foods[j]
            
            dist = abs(x1 - x2) + abs(y1 - y2)

            # Si ligne droite entre 2 points
            if x1 == x2 or y1 == y2:
                for k in range(min(x1, x2), max(x1, x2) + 1):
                    for l in range(min(y1, y2), max(y1, y2) + 1):
                        # Si un mur bloque la ligne droite
                        if walls[k][l]:
                            dist += 2
                            break
            ...
            for y in range(food.height):
                if food[x][y]:
                    prob = PositionSearchProblem(
                        gameState, start=startPosition, goal=(x, y), warn=False, visualize=False)
                    tmp = search.bfs(prob)
                    if len(tmp) < len(path) or len(path) == 0:
                        path = tmp
